...
def _metric_values(results, method, metric):
    return np.asarray([fold['methods'][method][metric] for fold in results['folds']], dtype=float)
...
def plot_results(results, out_path_prefix="../plots/results", alpha=None, epsilon=None, confounding_strength=None):
    methods = list(results['folds'][0]['methods'].keys())
    targets = [
        ('Y_0', 'cov_0', 'width_0'),
        ('Y_1', 'cov_1', 'width_1'),
    ...
    for target_id, (label, cov_key, width_key) in enumerate(targets):
        ...
        for box_group in (cov_boxes, width_boxes):
            ...
            for median in box_group['medians']:
...
def plot_treatment_realization_intervals(
    ...
    methods = ['GESPI', 'CCKE', 'NCCKE', 'Naive', 'CKE', 'WSCP_Inexact', 'WSCP_Exact']
    colors = {
        'GESPI': '#2f6fbb',
        'CCKE': '#c8524a',
        'NCCKE': '#4f8f5f',
    ...
    y0_plot = _as_2d_outcome(dataset_te['Y0'])
    y1_plot = _as_2d_outcome(dataset_te['Y1'])
    users = np.arange(y0_plot.shape[1])
    ...
    for local_id, original_idx in enumerate(chosen_idx):
        ...
        if target == 0 or target == 'all':
            plot_configs.append({
                ...
                'true': y0_plot[local_id],
                'sets': sets_0,
                'title': f"T=0 prediction intervals, sample {int(original_idx)}"
            })
        if target == 1 or target == 'all':
            plot_configs.append({
                'ax': axs[1] if target == 'all' else axs[0],
                'true': y1_plot[local_id],
                'sets': sets_1,
                'title': f"T=1 prediction intervals, sample {int(original_idx)}"
            })
        if target == 'ITE' or target == 'all':
            idx = 2 if target == 'all' else 0
            plot_configs.append({
                'ax': axs[idx],
                'true': y1_plot[local_id] - y0_plot[local_id],
                'sets': sets_ite,
        ...
        for config in plot_configs:
            ...
            ax.set_xlabel("Outcome / ITE")
            ax.set_ylabel("Outcome dimension")
            ax.set_yticks(users)

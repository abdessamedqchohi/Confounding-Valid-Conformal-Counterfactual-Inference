...
TARGETS = {
    "RR":   ("cov_0",   "width_0",   "RR (Round Robin)"),
    "PFCA": ("cov_1",   "width_1",   "PFCA (Proportional Fair)"),
    "ITE":  ("cov_ite", "width_ite", "ITE (Individual Treatment Effect)"),
}
...
def _draw_box_group(ax, grouped_values, confounding_strengths, ylabel):
    ...
    ax.set_xticklabels([rf"${c:g}$" for c in confounding_strengths])
...
def plot_sweep(runs, target, out_dirs, alpha=0.1, epsilon=0.025):
    ...
    for d in out_dirs:
        ...
        names = [
            f"combined_{target}",
            f"sweep_{target}_alpha{_fmt_param(alpha)}_eps{_fmt_param(epsilon)}_boxplot",
        ]
        for name in names:
    ...
    plot_single_column(confs, cov_data, width_data, target, out_dirs, target_level, guardrail_level, all_handles)

def plot_single_column(confs, cov_data, width_data, target, out_dirs, target_level, guardrail_level, handles):
    setup_latex_style()

    for metric_type, data, ylabel in [("coverage", cov_data, "Coverage"), ("width", width_data, "Efficiency")]:
        fig, ax = plt.subplots(figsize=(3.4, 2.5))
        _draw_box_group(ax, data, confs, ylabel=ylabel)
        if metric_type == "coverage":
            ax.axhline(target_level, color="black", linestyle="--", linewidth=1.1, alpha=0.85)
            ax.axhline(guardrail_level, color="#2f6fbb", linestyle=":", linewidth=1.2, alpha=0.9)
            ax.set_ylim(0.55, 1.03)

        fig.subplots_adjust(bottom=0.28, left=0.18, right=0.96, top=0.94)
        leg = fig.legend(
            handles=handles,
            loc="upper center",
        ...
        for d in out_dirs:
            pdf_path = os.path.join(d, f"{metric_type}_{target}.pdf")
            png_path = os.path.join(d, f"{metric_type}_{target}.png")
            fig.savefig(pdf_path, dpi=300, bbox_inches="tight", bbox_extra_artists=(leg,))
            fig.savefig(png_path, dpi=300, bbox_inches="tight", bbox_extra_artists=(leg,))
...
def main():
    ...
    out_dirs = [
        ...
        os.path.join(root, "plots_sweep_boxplot"),
    ]
